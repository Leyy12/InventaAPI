                      ...
                      <td className="p-4 align-top min-w-[160px]">
                        ...
                        <div className="mt-2">
                          <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                            <span>{entitlement?.status === 'upgrade_required' ? 'Protected API paused' : `${entitlement?.used ?? '—'} used (${entitlement?.period || 'unverified'})`}</span>
                            <span>{entitlement?.status === 'upgrade_required' ? 'Upgrade Required' : entitlement ? (entitlement.limit === null ? 'Unlimited' : entitlement.limit) : '—'}</span>
                          </div>
                          ...
                          {entitlement?.status === 'upgrade_required' && <p className="text-xs text-amber-300">No current Free API allowance; historical Trial usage is shown above.</p>}
