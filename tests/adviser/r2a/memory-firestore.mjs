...
export function memoryFirestore(initial = {}, { queryConflicts = true } = {}) {
  ...
  const collection = (name, filters = [], maximum = Infinity, cursor = '') => ({
    name,
    orderBy: () => collection(name, filters, maximum, cursor),
    startAfter: value => collection(name, filters, maximum, value),
    ...
    where: (field, op, value) => {
      ...
      return collection(name, [...filters, [field, value]], maximum, cursor);
    },
    limit: value => collection(name, filters, value, cursor),
    get: async () => {
      if (name.startsWith(db.failRead || '\0')) throw new Error('query unavailable');
      const docs = [...records.keys()].filter(path => path.split('/')[0] === name).sort().map(snapshot)
        .filter(doc => doc.id > cursor)
  ...
  db.seed = (path, data) => { revision++; records.set(path, { version: (records.get(path)?.version || 0) + 1, data: structuredClone(data) }); };
  db.remove = path => { revision++; records.delete(path); };
  db.runTransaction = async callback => {
    for (let attempt = 0; attempt < 100; attempt += 1) {
      const reads = new Map();
      let queryRevision = null;
      const writes = [];
      ...
      if (writes.length) { db.commits++; revision++; }
      return result;
    }
