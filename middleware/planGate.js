...
const PLAN_HIERARCHY = {
  ...
  'freetrial': 0,
  'FreeTrial': 0,      // 7-day trial — same access level as Free
  'pro': 1,
  'Pro': 1,
  ...
  try {
    ...
    if (userPlan === 'FreeTrial') {
      ...
      if (trialExpiresAt && now > trialExpiresAt) {
        // Lazily revoke the key so the user sees the error immediately
        try {
          ...
          await getDb().collection('users').doc(req.apiKeyData.userId).update({
            plan:            'Free',
            apiRequestLimit: 50,
      ...
      // 2. Check absolute item (request) quota
      const keyDoc2 = await getDb().collection('api_keys').doc(req.apiKeyData.id).get();
      const requestsUsedSoFar = keyDoc2.exists ? (keyDoc2.data().requestsUsed || 0) : 0;
      if (requestsUsedSoFar >= FREE_TRIAL_ITEM_QUOTA) {
        return res.status(429).json({
          ...
          quota: {
            limit:     FREE_TRIAL_ITEM_QUOTA,
            used:      requestsUsedSoFar,
            remaining: 0,
          },
          upgrade_url: UPGRADE_URL,
        });
